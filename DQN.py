...
class DQN:
    def __init__(self, Net, param, config, n_action, state_shape):
        ...
        try:
            self.eval_model.load_state_dict(torch.load("Save/" + config["SAVE_LOC"], map_location=torch.device('cpu')))
            self.target_model.load_state_dict(torch.load("Save/" + config["SAVE_LOC"], map_location=torch.device('cpu')))
